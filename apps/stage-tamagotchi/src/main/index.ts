...
import { startAlbionSidecars, stopAlbionSidecars } from './services/airi/albion-sidecars'
...
import { setupChatboxWindowManager } from './windows/chatbox'
...
app.whenReady().then(async () => {
  ...
  // 模块A「语音」：壳自己拉起本机 TTS/STT（端口已在跑就跳过；ALBION_SIDECARS=0 关掉）
  void startAlbionSidecars()
  ...
  const mainWindow = injeca.provide('windows:main', {
    ...
    build: async ({ dependsOn }) => setupMainWindow({
      ...
      onWindowCreated: (window) => {
        ...
        // 阿尔比恩桌宠：鼠标穿透已按要求移除（2026-09-14，指挥官：「可以不用了」）。
        // 需要恢复时：window.setIgnoreMouseEvents(true, { forward: true }) 即可。
        log.log('鼠标穿透：未启用（已移除）')
      },
    }),
  })
  ...
  const chatboxWindow = injeca.provide('windows:chatbox', {
    dependsOn: { widgetsManager, serverChannel, mcpStdioManager, i18n },
    build: async ({ dependsOn }) => setupChatboxWindowManager(dependsOn),
  })

  const tray = injeca.provide('app:tray', {
    dependsOn: { mainWindow, settingsWindow, captionWindow, chatboxWindow, widgetsWindow: widgetsManager, serverChannel, beatSyncBgWindow: beatSync, aboutWindow, i18n },
...
async function handleAppExit() {
  ...
  // 只杀本进程拉起来的本机语音服务；用户自己在别的窗口跑的实例不动
  stopAlbionSidecars()
