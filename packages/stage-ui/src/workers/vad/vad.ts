...
export class VAD implements BaseVAD {
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      // 本地模型：src/renderer/public/models/silero-vad/onnx/model.onnx
      // （原来从 HuggingFace 在线拉，本机网络不可达 → vad-init-failed → 报错弹窗）
      env.allowRemoteModels = false
      env.allowLocalModels = true
      env.localModelPath = '/models/'

      // Full-precision
      this.model = await AutoModel.from_pretrained('silero-vad', { config: { model_type: 'custom' } as any, dtype: 'fp32' })
      this.isReady = true
