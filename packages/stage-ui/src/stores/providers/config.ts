...
import { albionAddedProviders, albionProviderDefaults } from '../../albion-defaults'
...
export const useProviderConfigStore = defineStore('provider-config', () => {
  const providers = useLocalStorage<Record<string, InferenceServiceProvider>>('settings/providers/configured',
    albionProviderDefaults() as Record<string, InferenceServiceProvider>, providerStorageOptions)
  const addedProviders = useLocalStorage<Record<string, boolean>>('settings/providers/added', albionAddedProviders(), providerStorageOptions)
