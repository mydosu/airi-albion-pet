...
import { ALBION_STT_MODEL, ALBION_STT_PROVIDER_ID } from '../../albion-defaults'
...
export const useHearingStore = defineStore('hearing-store', () => {
  ...
  // State
  // 阿尔比恩桌宠：默认就接上本机 whisper「耳朵」，并且听到就自动发送（免耳机对话）
  const activeTranscriptionProvider = useLocalStorageManualReset('settings/hearing/active-provider', ALBION_STT_PROVIDER_ID, persistenceOptions)
  const activeTranscriptionModel = useLocalStorageManualReset('settings/hearing/active-model', ALBION_STT_MODEL, persistenceOptions)
  const activeCustomModelName = useLocalStorageManualReset('settings/hearing/active-custom-model', ALBION_STT_MODEL, persistenceOptions)
  const transcriptionModelSearchQuery = refManualReset<string>('')
  const autoSendEnabled = useLocalStorageManualReset<boolean>('settings/hearing/auto-send-enabled', true, persistenceOptions)
