...
export const useChatStore = defineStore('chat', () => {
  ...
  async function streamWithStageAdapters(
    ...
    // 以前是把壳这边的整段历史（带 [时间戳]、连 "User encountered error: …" 都算）回灌，
    // 与 Hermes 自己的会话历史形成双份上下文。壳本地 messages 不动 → 界面里照样能翻旧对话。
    ...
    const llmSpan = startSpan(IOSpanNames.LLMInference, activeTurnSpan.value, {
      ...
      [IOAttributes.LLMInputMessageCount]: outboundMessages.length,
      [IOAttributes.LLMInputUserMessageCount]: outboundMessages.length,
      [IOAttributes.TurnId]: options?.requestCorrelation?.roundId ?? '',
    })
    llmSpan.setAttribute(IOAttributes.LLMInputMessageRoles, outboundMessages.map(message => message.role))
    const llmRequestTs = performance.now()
    let llmFirstTokenEmitted = false

    try {
      await llmStore.stream(model, chatProvider, outboundMessages, {
        ...options,
        headers,
