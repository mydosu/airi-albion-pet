...
import { ALBION_STAGE_MODEL_ID } from '../../albion-defaults'
import { DisplayModelFormat, useDisplayModelsStore } from '../display-models'

export type StageModelRenderer = 'live2d' | 'vrm' | 'spine' | 'spine38' | 'tachie' | 'mmd' | 'godot' | 'disabled' | undefined
...
const useStageModelSelectionStore = defineStore('settings-stage-model-selection', () => {
  ...
  const selected = useLocalStorageManualReset<string>('settings/stage/model', ALBION_STAGE_MODEL_ID, {
...
export const useSettingsStageModel = defineStore('settings-stage-model', () => {
  ...
  const defaultStageModelId = ALBION_STAGE_MODEL_ID
  ...
  function resolveBuiltInStageModelRenderer(model?: DisplayModel): BuiltInStageModelRenderer {
    ...
    switch (model.format) {
      ...
      case DisplayModelFormat.SpineZip:
        return 'spine'
      case DisplayModelFormat.Spine38Zip:
        return 'spine38'
