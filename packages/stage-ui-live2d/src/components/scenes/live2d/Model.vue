...
async function performModelLoad() {
  ...
  try {
    ...
    // 阿尔比恩桌宠（本 fork）：直接传「以 .zip 结尾的 URL 字符串」给 pixi。
    // 上游传的是 `{ url, id }`，靠 AIRI 的 OPFS 中间件解包；而该注册在
    // 这个 pixi 版本下是死代码（它按 `middlewares.indexOf(ZipLoader.factory)` 找插入点，
    // 运行时找不到），结果对象一路透传到 jsonToSettings → `Unknown settings format`。
    // 字符串形态正是 pixi ZipLoader.factory 原生支持的分支，本地读取也不需要缓存层。
    ...
    // 阿尔比恩：交互层拖出来的参数（丝袜/孔明灯/相机…）要每帧写回，否则会被空闲动作覆盖
    motionManagerUpdate.register((ctx) => {
      const locked = (globalThis as any).__albionLockedParams
      if (!locked)
        return
      for (const [id, value] of Object.entries(locked))
        ctx.model.setParameterValueById(id, value as number)
    }, 'final')
    ...
    // 阿尔比恩：动作播完广播一次，桌宠模块据此接上「触摸后空闲」（手游就是这么串的）
    motionManager.on('motionFinish', () => {
      window.dispatchEvent(new CustomEvent('albion:motion-finish'))
    })
...
async function setMotion(motionName: string, index?: number) {
  ...
  // 阿尔比恩的模型没有 Happy/Sad/Think/... 这些情绪动作组（它只有 idle/idle1-5、
  // main_1-3、touch_*、login/mail/wedding 这些）。AIRI 按情绪名点播时，名字不存在就
  // 直接跳过——否则 pixi 每次都空跑一条 "Setting motion: Think" 的日志，还会把
  // 正在播的动作顶掉。情绪表情我们是用姿态参数（pose）演的。
  const definitions = model.value.internalModel?.motionManager?.definitions
  if (definitions && !(motionName in definitions)) {
    console.info('[albion] 模型没有这个动作组，跳过:', motionName)
    return
  }
...
// 桌宠互动层（我们的 fork）：把模型交给 window，用于网格级命中判定（albion-pet.ts）
watch(model, (m) => { (globalThis as unknown as Record<string, unknown>).__albionModel = m }, { immediate: true })
