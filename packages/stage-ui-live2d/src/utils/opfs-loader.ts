...
export class OPFSCache {
  ...
  static checkMiddleware: Middleware<OPFSContext> = async (context, next) => {
    ...
    // 阿尔比恩桌宠（本 fork）：**不走缓存命中路径**。
    // 命中时 OPFSCache.get() 返回的是整包解压出的 `File[]`，而下游 pixi 的
    // ZipLoader.factory 只认「长度 1、文件名以 .zip 结尾的 File」；不匹配就原样透传，
    // 最终 jsonToSettings 收到数组 → `Unknown settings format`（模型选得中、但舞台报错）。
    // 本地单模型每次 fetch 26MB 可接受；上游修好命中路径后可恢复缓存。
    const files = null
